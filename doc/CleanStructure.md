...
Но со всей этой информацией надо обязательно последовательно! ознакомиться.
...
В статье [Обратная сторона SCRUM](https://habr.com/ru/companies/kaiten/articles/891930/) прозвучало мнение:
```
«Если в команде собраны высокооплачиваемые эксперты, то формального лидера будет достаточно.
Команда самоорганизуется и достигнет результата, даже если не будет соблюдать все ритуалы.
Если команды только сформированы, в ней много джунов и мидлов, то нужно четко контролировать соблюдение процессов. 
Иначе самоорганизация превратится в хаос» — Дмитрий Ирешев.
```

Я перефразирую это высказывание:
```
Если в команде собраны эксперты, то они и так знают как надо правильно организовать структуру проекта. 
Им не нужны формальные правила и жесткая структура папок.
...
- какие модули есть в программном продукте?
- как тесно они между собой связаны?
- есть ли планы по-дальнейшему развитию? а по каждому модулю? 
...
Но однозначно могу сказать, качественный и легко поддерживаемый продукт не получится создать без разделения кода на зоны ответственности и написания тестов.
...
- **Query** - получение данных из внешних систем
- **Command** - изменение данных во внешних системах
- **DTO** - упрощенно, это "типизированный ассоциативный массив".
  **_Формальное правило_**: private и protected методы в классе могу возвращать ассоциативный массив, но public метод должен возвращать объект (массив объектов)
- **ValueObject** - DTO для описания узкоспециализированных данных, часто с встроенной валидацией данных 
- **Entity** - DTO для описания структуры таблицы в БД, требуется для ORM
...
- **/src**
  - **Core** - Ядро приложения, то от чего зависит работа большинства модулей, и код можно вынести в самостоятельную composer-библиотеку.
    Структура аналогична **компоненту**
    ...
    - **/%Название Подмодуля/Компонента%**
      - /**Application** - Реализация любой бизнес-логики
        ...
        - /**Responder** - Всё что требует специальной генерации контента вместо json для ответа пользователю
          - /**Template** - xml-шаблоны, Excel/Word-файлы-шаблоны, сообщения в Telegram
      ...
      - /**Domain** - Предметная область. Обычно все эти объекты доступны во вне модуля, как публичное API модуля (Контракт) 
        ...
        - /**Entity** - Dto для описания структуры таблицы, используется в **Command** для создания записи в БД
...
Но через один-два месяца, там появляется необходимость проверять доступность для Redis, RabbitMQ или ClickHouse.
...
Вся наша жизнь - это борьба в поиске и соблюдении баланса.
Таков путь!
